  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-slate-500 font-medium tracking-tight italic">Immutable registry of all mission-critical platform events and administrative overrides.</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <Select value={actionFilter} onValueChange={setActionFilter}>
          <SelectTrigger className="w-[200px] h-12 rounded-2xl border-slate-100 font-bold text-xs uppercase tracking-widest text-slate-600">
            <SelectValue placeholder="Event Type" />
          </SelectTrigger>
          <SelectContent className="rounded-xl border-slate-100 p-2">
            <SelectItem value="ALL" className="rounded-lg font-bold">All Events</SelectItem>
            <SelectItem value="UPDATE_SUBSCRIPTION" className="rounded-lg font-bold text-amber-600">Subscription Override</SelectItem>
            <SelectItem value="APPROVE_ORGANISATION" className="rounded-lg font-bold text-emerald-600">Org Approvals</SelectItem>
            <SelectItem value="SUSPEND_USER" className="rounded-lg font-bold text-red-600">Identity Suspensions</SelectItem>
            <SelectItem value="IMPERSONATE" className="rounded-lg font-bold text-indigo-600">Admin Impersonation</SelectItem>
          </SelectContent>
        </Select>
      </div>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-32 gap-4">
              ...
              <p className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] animate-pulse">Syncing immutable ledgers...</p>
            </div>
          ) : !filteredLogs || filteredLogs.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-32 text-center space-y-4">
              <div className="p-5 bg-slate-50 text-slate-300 rounded-[2rem]">
                <Activity size={48} />
              </div>
              <div>
                <p className="font-black text-slate-900 uppercase tracking-tight text-xl">No Events Recorded</p>
                <p className="text-sm text-slate-400 font-medium">Platform activity logs will appear here as they occur.</p>
              </div>
            </div>
          ) : (
            <Table>
      ...
      <div className="p-6 bg-indigo-900 rounded-[2rem] text-white overflow-hidden relative group">
        ...
        <div className="relative z-10 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-black uppercase tracking-[0.3em] text-indigo-200">Legal Compliance Registry</h3>
          </div>
          <p className="text-xl font-bold max-w-2xl leading-relaxed">
            This audit trail is <span className="text-indigo-300 italic underline decoration-indigo-400/50">immutable</span>. All entries are hashed and stored with co-founder identity markers to ensure absolute platform governance.
          </p>
        </div>
      </div>
    </div>
