    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-0">
          <Table>
            <TableHeader><TableRow className="bg-primary/5"><TableHead>{t('legacy.invoiceNumber')}</TableHead><TableHead>{t('legacy.amount')}</TableHead><TableHead>{t('legacy.balance')}</TableHead><TableHead>{t('legacy.status')}</TableHead><TableHead>{t('legacy.due')}</TableHead></TableRow></TableHeader>
            <TableBody>
              {(invoices ?? []).map(inv => (
