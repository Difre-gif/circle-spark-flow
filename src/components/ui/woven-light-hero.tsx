        ...
        >
            <div className="max-w-7xl mx-auto flex justify-between items-center">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xl font-bold text-slate-800" style={{ fontFamily: "'Inter', sans-serif" }}>Woven</span>
                </div>
            </div>
        </motion.nav>
    );
...
const WovenCanvas = () => {
  ...
  useEffect(() => {
    ...
    // const isDarkMode = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches; // Removed dark mode check
    ...
    for (let i = 0; i < particleCount; i++) {
        ...
        // Adjusted for light theme - subtle gray/dark particles
        color.setHSL(Math.random(), 0.1, 0.4);
        colors[i * 3] = color.r;
    ...
    const material = new THREE.PointsMaterial({
        size: 0.02,
        vertexColors: true,
        blending: THREE.NormalBlending, // Force NormalBlending for light theme
        transparent: true,
        opacity: 0.6, // Adjusted opacity for light theme
    });
...
interface WovenLightHeroBackgroundProps {
  children?: React.ReactNode;
}

export const WovenLightHeroBackground: React.FC<WovenLightHeroBackgroundProps> = ({ children }) => {
  return (
