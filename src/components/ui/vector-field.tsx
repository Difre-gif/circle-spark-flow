...
export function VectorField({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Clear canvas with light background (e.g., #F8FAFC)
      ...
      for (let x = 0; x < gridSize; x++) {
        for (let y = 0; y < gridSize; y++) {
          ...
          if (point.locked) {
            // Subtle dark blue/gray for locked points
            gradient.addColorStop(0, `rgba(70, 90, 120, ${fadeAmount * 0.8})`)
            gradient.addColorStop(1, `rgba(90, 110, 140, ${fadeAmount})`)
          } else {
            // Lighter gray for unlocked points
            gradient.addColorStop(0, `rgba(180, 180, 180, ${fadeAmount * 0.6})`)
            gradient.addColorStop(1, `rgba(200, 200, 200, ${fadeAmount})`)
          }
          ...
          ctx.strokeStyle = point.locked ? `rgba(70, 90, 120, ${arrowOpacity})` : `rgba(180, 180, 180, ${arrowOpacity})`
          ...
          if (point.locked && frameCountRef.current % 3 === 0) {
            ...
            ctx.fillStyle = `rgba(70, 90, 120, ${fadeAmount * 0.3})`
            ctx.fill()
          }
        }
      }
...
interface VectorFieldBackgroundProps {
  ...
  preset?: 'large' | 'small' | 'dense';
  // Allow overriding individual props for VectorField as well
  gridSize?: number;
  maxVector?: number;
  padding?: number;
  margin?: number;
...
export const VectorFieldBackground: React.FC<VectorFieldBackgroundProps> = ({ children, preset, ...props }) => {
  const renderVectorField = () => {
    switch (preset) {
      case 'large':
        return <LargeVectorField {...props} />;
      case 'small':
        return <SmallVectorField {...props} />;
      case 'dense':
        return <DenseVectorField {...props} />;
      default:
