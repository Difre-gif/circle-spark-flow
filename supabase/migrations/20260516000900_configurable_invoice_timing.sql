-- Let landlords configure when invoices are sent:
-- 1) workspace-wide default lead time before due date
-- 2) optional per-tenant override

ALTER TABLE public.tenancies
  ADD COLUMN IF NOT EXISTS invoice_lead_days integer CHECK (invoice_lead_days BETWEEN 0 AND 30);
...
SET settings = jsonb_set(
  settings,
  '{billing,default_invoice_lead_days}',
  to_jsonb(COALESCE((settings->'billing'->>'default_invoice_lead_days')::int, 7)),
  true
);
...
BEGIN
  ...
  IF v_inv.unit_id IS NOT NULL AND v_inv.role = 'TENANT' THEN
    SELECT monthly_rent INTO v_monthly_rent FROM units
    ...
    INSERT INTO tenancies (
      org_id, unit_id, tenant_user_id, start_date, agreed_rent, deposit_amount,
      status, created_by, billing_frequency, period_anchor_day, invoice_lead_days
    )
    SELECT
      v_inv.org_id,
      v_inv.unit_id,
...
BEGIN
    ...
    LOOP
        ...
        ELSIF current_date = (v_anchor_this_month - (v_invoice_lead_days || ' days')::interval)::date THEN
            v_target_due_date := v_anchor_this_month;
        ELSIF current_date = (v_anchor_next_month - (v_invoice_lead_days || ' days')::interval)::date THEN
